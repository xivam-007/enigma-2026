        ...
        {cards.map((card) => {
          ...
          return (
            <Link href={targetUrl} key={card.id}>
              <div
                className={`flex items-center p-6 border rounded-xl shadow-sm transition-all cursor-pointer ${card.color}`}
              >
                <div
                  className={`p-4 rounded-full bg-white mr-4 shadow-sm ${card.iconColor}`}
                >
                  <Icon size={32} />
                </div>
                <div>
